...
const { forgotPasswordValidator, resetPasswordValidator } = require('../validators/user');
...
router.post('/changePassword', checkLogin, resetPasswordValidator, async function(req, res, next) {
  ...
  try {
    ...
    const validPassword = await user.isValidPassword(req.body.oldPassword);
    if (!validPassword) {
